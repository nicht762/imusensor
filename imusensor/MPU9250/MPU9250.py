...
class MPU9250:
    ...
    def readSensor(self):
        ...
        self.AccelVals = (np.squeeze(self.cfg.transformationMatrix.dot(
            vals[np.newaxis, :3].T)) * self.AccelScale - self.AccelBias) * self.Accels
        self.GyroVals = np.squeeze(
            self.cfg.transformationMatrix.dot(vals[np.newaxis, 4:7].T)) * self.GyroScale - self.GyroBias
    ...
    def caliberateAccelerometer(self):
        ...
        print("Acceleration calibration is starting and keep placing the IMU in 6 different directions based on the "
              "instructions below")
        time.sleep(2)
        for i in range(6):
        ...
        if len(xscale) != 2 or len(yscale) != 2 or len(zscale) != 2:
            print("It looks like there were some external forces on sensor and couldn't get proper values. Please try "
                  "again")
            return
    ...
    def computeOrientation(self):
        ...
        self.yaw = np.arctan2(np.sin(self.roll) * normMagVals[2] - np.cos(self.roll) * normMagVals[1],
                              np.cos(self.pitch) * normMagVals[0] + np.sin(self.roll) * np.sin(self.pitch) *
                              normMagVals[1] +
